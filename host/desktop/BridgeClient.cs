...
public sealed class BridgeClient
{
    ...
    {
        int code = 0;
        var current = session = new TradeSession(Emit);
        try { await Task.Factory.StartNew(() => current.Run(port, party, selected, run, cancel), cancel, TaskCreationOptions.LongRunning, TaskScheduler.Default); }
        catch (OperationCanceledException) when (cancel.IsCancellationRequested) { }
        catch (Exception error) { code = 1; Emit(new { @event = "error", message = error.Message }); }
        finally
        {
            Running = false; session = null; cancellation?.Dispose(); cancellation = null;
            Emit(new { @event = "disconnected" }); Exited?.Invoke(code);
        }
    }
    public void CancelTrade() => session?.DeclineTrade();
    public void Offer(int slot) => session?.OfferSlot(slot);
