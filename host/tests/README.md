...
`Frlg.Trade.Tests` doubles as the console front end on platforms without the WPF desktop app.
...
The standby party lives in `local/party.json`, the same file and format the desktop app uses
(`selected` index plus six hex PK3 slots). Without that file the two default assets
(Mewtwo, Deoxys) are used and Deoxys is offered. A trade needs at least two Pokémon.

`--live` writes each session to `local/runs/<timestamp>-native-console/`: `offered.pk3` (the Pokémon
sent), `received.pk3` (the one received, saved at commit), `native.log` and the `pia.jsonl` capture.
