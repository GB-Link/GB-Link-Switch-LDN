/* Host-side simulation of the post-trade standby exchange between a retail
   FireRed cartridge (child) and the Switch's NSO build (parent), run through
   trade_shim. Build and run from firmware/esp32-s3:

       cc -std=c11 -Wall -Wextra -I main tools/trade_shim_test.c main/trade_shim.c -o /tmp/trade_shim_test && /tmp/trade_shim_test

   The models follow pokefirered's link_rfu_2.c: a numbered READY_EXIT_STANDBY
   barrier whose commands are ignored unless the number equals the receiver's
   own counter, ready flags that are set whenever a matching command arrives and
   cleared only when a round completes, the leader sending its own command twice
   and never again, the child retrying every 60 frames, the child's link layer
   refusing a block request while a barrier or a previous block send is active
   (and the parent asking only once), and the parent validating a consecutive
   mod-8 tag on every child command. The scripts follow trade.c /
   trade_scene.c: the retail child runs five standby rounds after a trade, the
   NSO parent six. Fault knobs drop frames the way the Switch emulator and the
   adapter path have been seen to. */
...
static void handle(actor_t *a, int i, const slot_t *s, int64_t now)
{
    ...
    if (cmd == 0) return;
    if (i != (a->parent ? 0 : -1)) a->last_peer_command = now;   /* anything not our own */
