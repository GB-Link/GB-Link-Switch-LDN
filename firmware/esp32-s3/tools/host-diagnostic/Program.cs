...
if (args.Length != 2 || args[0] is not ("serial" or "auth" or "pia" or "room")) {
    Console.Error.WriteLine("Usage: S3.Diagnostic <serial|auth|pia|room> COM<number>"); return 2;
}
...
try {
    ...
    if (device.Model != "esp32s3") throw new InvalidOperationException("Expected an ESP32-S3 device");
    Console.WriteLine("S3 protocol v1 handshake and session begin: OK");
    ...
    void Status() { foreach (string line in device.Command("LDN_STATUS")) if (line.StartsWith("LDN_S3_STATS")) Console.WriteLine(line); }
    Status();
    ...
    while (join.Elapsed.TotalSeconds < 40 && !(accepted && membership != null)) {
        ...
        if (now >= nextStatus) {
            foreach (string line in device.Command("LDN_STATUS")) {
                ...
                if (line.StartsWith("LDN_S3_STATS")) Console.WriteLine(line);
            }
            nextStatus = now + 2;
        }
    ...
    if (args[0] is "pia" or "room") {
        ...
        string run = Path.Combine(root.FullName, "local", "esp32-s3", "runs", DateTimeOffset.UtcNow.ToUnixTimeMilliseconds().ToString());
