...
if ($Action -ne 'build' -and $Port -notmatch '^COM[1-9][0-9]*$') { throw 'Specify the ESP32 port with -Port COM<number>.' }
...
if ($Action -eq 'flash') {
    # esptool checks the actual chip against the ESP32 image before writing.
    Invoke-Esp32Python ($idfArguments + @('-p', $Port, '-b', '460800', 'flash'))
}
Write-Host "ESP32 firmware: $(Join-Path $build 'ldn_esp32_bridge.bin')"
