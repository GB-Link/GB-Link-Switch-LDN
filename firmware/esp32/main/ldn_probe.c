...
#error "This independent firmware requires the original ESP32"
#endif
...
static void install_ldn_keys(void)
{
    ...
    /* Readback is diagnostic only (LDN_KEY_STATUS reports what the driver returns); the encrypted
       LDN authentication exchange is the check that matters. */
