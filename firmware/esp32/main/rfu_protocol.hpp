// Vendored verbatim from GBLink-Firmware (branch wireless-bridge, fw 2.4.20):
//   src/sections/rfuProtocol.hpp
//
// The RFU adapter state machine is pure C++ (<cstdint>/<cstring>/<cstddef> only)
// and carries the hardware-proven protocol behaviour, so it is used unmodified.
// Re-vendor from upstream rather than editing this copy.
            ...
            case NET_HOST_SEND:
                if (state == stClient)
                {
                    ...
                    if (plen >= blen)
                    {
                        ...
                        // stream: the real link overwrites, it never queues.
                        // A parent that is never silent (its idle frames still
                        // carry the LLSF header) fills this FIFO the moment
                        // our game pauses its pump for a save or a screen
                        // transition, and since it refills as fast as the game
                        // drains, every later frame arrives 32 frames stale:
                        // the games' link-standby handshake then never arms.
                        // So a UNI frame behind an unconsumed UNI frame
                        // replaces it; NI frames (sequenced, acked) still
                        // queue in full order.
...
private:
    ...
    // state: it may be the only copy of something the game must act on (a link-standby round, a block
    // request), and the peer sends it once before reverting to idle. Dropping or overwriting one strands
    // the game's handshake forever -- the post-trade standby wall was exactly this, because the GBA polls
    // slowly while writing its save and the command was replaced by the next idle before it was read.
