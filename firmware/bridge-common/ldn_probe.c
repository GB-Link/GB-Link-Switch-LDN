...
#error "This independent firmware runs on the ESP32-S3 and ESP32-C6"
#endif
...
int8_t ldn_probe_last_rssi(void) { return s_last_ldn_rssi; }

/* One sample is noise; a Switch a few metres away should sit well above -60 dBm,
   and a radio on a dead antenna path reads weak and jittery whatever the distance. */
void ldn_probe_rssi_window(int *count, int *average, int *low, int *high)
{
    *count = s_rssi_count;
    *average = s_rssi_count ? s_rssi_sum / s_rssi_count : 0;
    *low = s_rssi_count ? s_rssi_min : 0;
    *high = s_rssi_count ? s_rssi_max : 0;
    s_rssi_count = s_rssi_sum = 0;
    s_rssi_min = s_rssi_max = 0;
}

static void note_rssi(int8_t rssi)
{
    s_last_ldn_rssi = rssi;
    if (s_rssi_count == 0) { s_rssi_min = s_rssi_max = rssi; }
    else if (rssi < s_rssi_min) s_rssi_min = rssi;
    else if (rssi > s_rssi_max) s_rssi_max = rssi;
    s_rssi_sum += rssi;
    ++s_rssi_count;
}
...
static bool read_key_back(int expected_index, enum ldn_key_flag flag,
                          const uint8_t *expected, int *driver_result)
{
...
static void install_ldn_keys(void)
{
    ...
    /* Readback is diagnostic only: the pinned S3 driver accepts both keys but reads back neither
       ...
       authentication exchange is the check that matters. */
    ...
    printf("LDN_KEY_STATUS pairwise_result=%d group_result=%d pairwise_readback=%u group_readback=%u readback_results=%d,%d\n",
           pairwise, group, pairwise_ok, group_ok, pairwise_read, group_read);
    if (pairwise != 0 || group != 0) {
        ESP_LOGE(TAG, "CCMP key install failed; not authorizing port");
        return;
    }
...
static void run_private_join(void)
{
    ...
    /* This loop carries the only work with a deadline: the 59.727 Hz tick and draining
       the socket the Switch's frames arrive on. At the main task's priority 1 it sits
       under the Pico UART pump at 10, which on one core lets the pump -- 4 KB of driver
       buffer, 44 ms of slack -- preempt a clock with 16.7 ms. Where there are two cores
       the pump is pinned to the other one and the question never arises. Stay below the
       Wi-Fi and TCP/IP tasks (23 and 18), and yield every iteration so the pump still
       runs in the gaps. */
...
/* Boards that must configure something before the radio runs -- an antenna switch,
   a power rail -- define this; the default is for boards that need nothing. */
