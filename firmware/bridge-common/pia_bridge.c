...
static struct
{
    ...
    uint32_t poll_gap_max_us, poll_gap_late;
} g;
...
void pia_bridge_poll(void)
{
    if (!g.started) return;
    int64_t now = now_ms();
    /* Everything with a deadline -- the 59.727 Hz tick, draining the UDP socket the
       Switch's frames land in -- happens on this one call. On a single-core part a
       higher-priority task keeping it off the CPU shows up as lost host frames and
       nothing else, so measure the gap rather than infer it. */
    {
        const int64_t poll_us = esp_timer_get_time();
        if (g.last_poll_us)
        {
            const uint32_t gap = (uint32_t)(poll_us - g.last_poll_us);
            if (gap > g.poll_gap_max_us) g.poll_gap_max_us = gap;
            if (gap > 16742) ++g.poll_gap_late;
        }
        g.last_poll_us = poll_us;
    }
...
void pia_bridge_status(char *out, size_t cap)
{
    ...
    snprintf(out, cap,
             ...
             "queue_high=%d poll_gap_max_us=%" PRIu32 " poll_gap_late=%" PRIu32 " rssi=%d",
             ...
             g.link.high_water, g.poll_gap_max_us, g.poll_gap_late, ldn_probe_last_rssi());
