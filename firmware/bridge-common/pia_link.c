...
void pia_link_enqueue(pia_link_t *l, const uint8_t *payload, size_t length)
{
    ...
    /* Collapse only an exact repeat of the frame queued last -- that is an RFU-level
    ...
    /* The GBA writes a held-keys report every frame and the Switch takes about one child
       frame per datagram it sends, fewer than that whenever it bundles its own frames,
       so while the player walks the reports arrive faster than they can leave. Both
       games move the GBA's avatar from the parent's echo of these reports, so a report
       dropped here -- before its sequence tag is stamped -- is dropped for both of them
       alike: the avatar just acts on the fresher input behind it. Anything that is not
       superseded, a button press or any trade traffic, stays queued in order. */
...
/* Wrap a child payload as a WT frame. */
...
/* The next child frame to send, or the idle frame when nothing new is waiting.
   Never repeat a real command: the games discard a frame only when every slot's
   command word is zero, so a command left on repeat keeps the peer's receive queue
   permanently non-empty and the standby handshake deadlocks. */
